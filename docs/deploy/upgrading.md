...
At this point, your yente is running entirely from the `yente-blue` indices. Your Eliastic will still have a bunch of of "old" `yente-green` indices around - you may delete them. Alternatively, the next time you upgrade, you can do the same procedure but switching from `yente-blue` to `yente-green`. Your old `yente-green` indices will be cleaned up during the first reindex.

### Upgrading from Elastisearch 8 to 9
...
1. Upgrade Elasticsearch to the latest version in the 8.x series in `docker-compose.yml` (at the time of writing, thats `8.19.13`, check [Docker Hub](https://hub.docker.com/_/elasticsearch) for the latest)
