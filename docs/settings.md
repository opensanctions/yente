...
| `YENTE_MATCH_FUZZY` | `true` | Whether `/match` candidate retrieval adds an edit-distance clause per query name part, so that typos and minor spelling variants are still retrieved. The clause looks up precomputed deletion variants of the indexed name parts, so it costs little more than an exact lookup. Exact, known-name symbol and space-less name matching are always on. |
