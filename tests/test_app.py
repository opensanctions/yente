...
def test_redoc_has_sri():
    ...
    assert f'integrity="{settings.REDOC_JS_SRI}"' in html, (
        "SRI integrity attribute missing"
    )
...
def test_match_without_content_type_header():
    ...
    res = client.post("/match/zala", content=json.dumps(query))
...
def test_max_url_length_rejects_oversized_query():
    ...
    res = client.get(f"/search/zala?q={long_q}")
...
def test_max_url_length_allows_normal_query():
    ...
    res = client.get("/search/zala?q=acme")
